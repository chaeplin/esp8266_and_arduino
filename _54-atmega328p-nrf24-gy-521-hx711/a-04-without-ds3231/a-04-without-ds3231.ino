...
void setup() {
  ...
  if (DEBUG_PRINT) {
    ...
    delay(100);
    Serial.println();
    Serial.println("00 --> Starting unit : pir, hx711, radio");
  }
  ...
  while (1) {
    ...
    if (( ave.stddev() < 20 ) && ( nofchecked > 8 ) && ( ave.mean() > 3000 ) && ( ave.mean() < 7000 ) && (!bavgsent)) {
      if (DEBUG_PRINT) {
        ...
        Serial.print(ave.mean() - emptyWeight);
        Serial.print(" stddev ===> : ");
        Serial.println(ave.stddev());
        delay(10);
      } else {
        scale_payload.avemean   = (int16_t)ave.mean() - emptyWeight;
    ...
    if ( nofchecked > 2 ) {
      if (DEBUG_PRINT) {
        ...
        if ( (nofchecked % 3) == 0 ) {
          scale_payload.avemean   = nWeight - emptyWeight;
          scale_payload.avestddev = (int16_t)ave.stddev();
...
void track_empty() {
  ...
  if (!bpir_isr) {
    ...
    scale_payload.avemean   = emptyWeight;
    scale_payload.avestddev = 0;
    scale_payload.avetype   = 0; // tare
    radio.powerUp();
    radio.write(&scale_payload, sizeof(scale_payload));
    radio.powerDown();
  }
