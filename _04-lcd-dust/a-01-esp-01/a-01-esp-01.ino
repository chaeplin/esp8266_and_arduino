...
volatile bool T2_UPDATE, OT_UPDATE, PW_UPDATE, NW_UPDATE, PIR_UPDATE, HO_UPDATE, HL_UPDATE, AC_UPDATE;
...
void parseMqttMsg(String receivedpayload, String receivedtopic) {
  ...
  if ( receivedtopic == substopic[6] ) {
    if (root.containsKey("AC")) {
      if (device_data.tempmode == 0) {
        device_data.tempmode = 1;
      } else {
  ...
  if ( receivedtopic == substopic[7] ) {
    if (root.containsKey("actemp")) {
      device_data.tempset = uint8_t(root["actemp"]);
    }
    if (root.containsKey("acflow")) {
      device_data.tempflow = uint8_t(root["acflow"]);
    }
  }
...
void setup() {
  ...
  T2_UPDATE = OT_UPDATE = PW_UPDATE = NW_UPDATE = PIR_UPDATE = HO_UPDATE = HL_UPDATE = AC_UPDATE = false;
  msgcallback = false;
  acquirestatus = 0;
        ...
        if (now() != prevDisplay) {
          ...
          if (haveData) {
            if (device_data.hash == calc_hash(device_data)) {
              moisture = device_data.moMeasured;
              float calcVoltage = device_data.voMeasured * (5.0 / 1024.0);
